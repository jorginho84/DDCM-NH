...
ALL ADULTS DATABASE (ONLY IN THE CFS)
...
*Children: KID7DATY kid*daty indicates Birth YEAR,Child number *\in {1,...,7} (YY)

forvalues x=1/7{

	label variable kid`x'daty "Birth year child # `x'"
}
...
*does not match and don't know the categories (Table 3.1 on year-2 report)
...
*Don't have earnings for the first quarter of 98!!
...
*CSJ: by month: do not coincide with the previous variable. what's going on?
forvalues x=1/24{
label variable csjm`x' "CSJ amount month `x'"
}
...
*AFDC
/*
note: the variable adfc_q1 (first quarter) uses adfm1-3. why?
*/
forvalues x=0/24{
	label variable adfm`x' "afdc amt fiscal month `x'"

}

forvalues x=1/8{
	label variable afdcq`x' "AFDC quarter `x'"

}


*W2 (Wisconsin-Work ) or AFDC 
forvalues x=1/37{
	label variable wwq`x' "AFDC or W-2 quarter `x'"
...
*Food stamps
forvalues x=0/24{
	label variable fsfm`x' "afdc amt fiscal month `x'"

}

forvalues x=1/37{
	label variable fsq`x' "Fodd stamp quarter `x'"

}

*Total income (earnings, AFDC, FS, Supp)

forvalues x= 1/31{
	label variable tincq`x' "Total income (earnings, AFDC, FS, Supp) quarter `x'"
}
...
/*
income from family sources: cannot identify members
question 101: in 1999, how many weeks did you work for pay
question 102: in those weeks, how many hours on average
question 103: earnings in 1999
question 104: from business, rent, dividens, and interests
question 105: government cash payments from W-2, welfare, unemplyment benefits, social security, SSI, others similar
question 106: all other sources, such as alimony, child support, help from friends or relatives
...
forvalues x=1/9{
	...
	label variable piq101_`x' "how many weeks did you work for pay"
	label variable piq102_`x' "how many hours a week on average"
	label variable piq103_`x' "earnings in 1999"
	label variable piq104_`x' "income from business, rent, dividens, and interests"
	label variable piq105_`x' "income from government cash payments from W-2, welfare, unemplyment benefits, social security, SSI, others similar"
	label variable piq106_`x' "income from all other sources, such as alimony, child support, help from friends or relatives"
...
*What was (expect) to be the gross family income in 2003?
