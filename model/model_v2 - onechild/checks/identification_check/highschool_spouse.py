...
for i in range(size_grid): 
	param0.beta_spouse[0,0] = p_list[i]
	emax_instance=output_ins.emax(param0,model)
	choices=output_ins.samples(param0,emax_instance,model)
	dic_betas=output_ins.aux_model(choices)
	target_moment[i] = np.mean(dic_betas['beta_wage_spouse'][0,:],axis=0)
...
ax.legend()
ax.set_ylabel(r'High school coefficient of spouse income',fontsize=font_size)
ax.set_xlabel(r'High school coefficient of spouse income',fontsize=font_size)
...
ax.legend(loc=0)
plt.show()
fig.savefig('/home/jrodriguez/NH_HC/results/model_v2/checks/highschool_spouse.pdf', format='pdf')
plt.close()
