...
for i in range(size_grid): 
	param0.beta_spouse[2,0] = p_list[i]
	emax_instance=output_ins.emax(param0,model)
	choices=output_ins.samples(param0,emax_instance,model)
	dic_betas=output_ins.aux_model(choices)
	target_moment[i] = np.mean(dic_betas['beta_wage_spouse'][2,:],axis=0)
...
ax.legend()
ax.set_ylabel(r'$\sigma^2$ wage regression (spouse)',fontsize=font_size)
ax.set_xlabel(r'$\sigma^2$ of wage offer process (spouse)',fontsize=font_size)
...
ax.legend(loc=0)
plt.show()
fig.savefig('/home/jrodriguez/NH_HC/results/model_v2/checks/sigma2_spouse.pdf', format='pdf')
plt.close()
