...
for i in range(size_grid): 
	param0.gamma3 = p_list[i]
	emax_instance=output_ins.emax(param0,model)
	choices=output_ins.samples(param0,emax_instance,model)
	dic_betas=output_ins.aux_model(choices)
	target_moment_original[i] = np.mean(dic_betas['beta_inputs'][2,:],axis=0)
...
for i in range(size_grid): 
	...
	dic_betas=output_ins.aux_model(choices)
	target_moment_noeitc[i] = np.mean(dic_betas['beta_inputs'][2,:],axis=0)
...
ax.set_ylim(-0.055,-0.030)
ax.set_xlim(0.14,0.3)
ax.legend(loc=0)
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/experiments/EITC/gamma3_check_eitc.pdf', format='pdf')
plt.close()

#the graph: this is for slides
fig, ax=plt.subplots()
plot1=ax.plot(p_list,target_moment_original,'b-o',label='Simulated',alpha=0.9)
plot3=ax.plot(p_list,np.full((size_grid,),obs_moment),'b-.',label='Observed',alpha=0.9)
plt.setp(plot1,linewidth=3)
plt.setp(plot3,linewidth=3)
ax.legend()
...
ax.legend(loc=0)
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/experiments/EITC/gamma3_check_eitc_1plot.pdf', format='pdf')
plt.close()