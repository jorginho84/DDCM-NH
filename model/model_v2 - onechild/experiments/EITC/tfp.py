...
for i in range(size_grid): 
	...
	target_moment_original[i] = np.mean(dic_betas['beta_inputs'][3,:],axis=0)
...
for i in range(size_grid): 
	...
	dic_betas=output_ins.aux_model(choices)
	target_moment_noeitc[i] = np.mean(dic_betas['beta_inputs'][3,:],axis=0)
...
ax.legend(loc=0)
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/experiments/EITC/tfp_check_eitc.pdf', format='pdf')
plt.close()