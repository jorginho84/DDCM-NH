...
for i in range(size_grid): 
	...
	target_moment[i] = np.mean(dic_betas['beta_inputs_young_cc1'][1,:],axis=0)
...
plot3=ax.plot(np.full((size_grid,),gamma1[0]),target_moment,'k--',label='Estimated parameter',alpha=0.9)
plt.setp(plot1,linewidth=3)
plt.setp(plot2,linewidth=3)
plt.setp(plot3,linewidth=3)
ax.legend(loc=4)
ax.set_ylabel(r'$Corr(l_t,SSRS_2)$')
ax.set_xlabel(r'Productivity of $\theta_t$ ($\gamma_1$, age $\leq 6$)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/gamma1_young.pdf', format='pdf')
plt.close()
