...
for i in range(size_grid): 
	...
	target_moment[i] = np.mean(dic_betas['beta_inputs_old'][0,:],axis=0)
...
plot3=ax.plot(np.full((size_grid,),gamma2[1]),target_moment,'k--',label='Estimated parameter',alpha=0.9)
...
ax.set_xlabel(r'Productivity of consumption ($\gamma_2$, age $>5$)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/gamma2_old.pdf', format='pdf')
plt.close()
