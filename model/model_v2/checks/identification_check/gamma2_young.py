...
lenght = 0.05
size_grid = 5
max_p = gamma2[0] + lenght
min_p = gamma2[0] - lenght
...
plot3=ax.plot(np.full((size_grid,),gamma2[0]),target_moment,'k--',label='Estimated parameter',alpha=0.9)
plt.setp(plot1,linewidth=3)
plt.setp(plot2,linewidth=3)
plt.setp(plot3,linewidth=3)
ax.legend()
ax.set_ylabel(r'$Corr(c_t, SSRS_5)$')
ax.set_xlabel(r'Productivity of consumption ($\gamma_2$, age $\leq 6$)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/gamma2_young.pdf', format='pdf')
plt.close()
