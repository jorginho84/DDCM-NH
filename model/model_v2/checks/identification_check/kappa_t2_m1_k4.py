...
lenght = 1.5
size_grid = 4
max_p = kappas[0][0][3] + lenght
min_p = kappas[0][0][3] - lenght
...
for i in range(size_grid): 
	...
	target_moment[i] = np.mean(dic_betas['beta_kappas_t2'],axis=2)[3][0] #The matrix is 4x3
...
plot3=ax.plot(np.full((size_grid,),kappas[0][0][3]),target_moment,'k--',label='Estimated parameter',alpha=0.9)
plt.setp(plot1,linewidth=3)
plt.setp(plot2,linewidth=3)
plt.setp(plot3,linewidth=3)
ax.legend(loc = 0)
ax.set_ylabel(r'Moment')
ax.set_xlabel(r'Cutoff ($\kappa_4$): Reading')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/kappa_t2_m1_k4.pdf', format='pdf')
plt.close()
...
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/qw_kappa_t2_m1_k4.pdf', format='pdf')
plt.close()
