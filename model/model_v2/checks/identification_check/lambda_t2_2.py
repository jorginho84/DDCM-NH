...
lenght = 3
size_grid = 4
max_p = lambdas[0][2] + lenght
min_p = lambdas[0][2] - lenght
...
for i in range(size_grid): 
	...
	target_moment[i] = np.mean(dic_betas['beta_lambdas_t2'],axis=1)[1]
...
plot3=ax.plot(np.full((size_grid,),lambdas[0][2]),target_moment,'k--',label='Estimated parameter',alpha=0.9)
plt.setp(plot1,linewidth=3)
plt.setp(plot2,linewidth=3)
plt.setp(plot3,linewidth=3)
ax.legend(loc = 0)
ax.set_ylabel(r'Moment')
ax.set_xlabel(r'Factor loading ($\lambda$): Int functioning')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/lambda_t2_intfunc.pdf', format='pdf')
plt.close()
