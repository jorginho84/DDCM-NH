...
lenght = 4
size_grid = 4
max_p = lambdas[1][0] + lenght
min_p = lambdas[1][0] - lenght
p_list = np.linspace(min_p,max_p,size_grid)
obs_moment = moments_vector[26,0].copy()
#obs_moment = 0.13

#draft: try updating a parameter
target_moment = np.zeros((size_grid,))
for i in range(size_grid): 
	param0.lambdas[1][0] = p_list[i]
	...
	target_moment[i] = np.mean(dic_betas['beta_lambdas_t5'],axis=1)[0]
...
plot3=ax.plot(np.full((size_grid,),lambdas[1][0]),target_moment,'k--',label='Estimated parameter',alpha=0.9)
plt.setp(plot1,linewidth=3)
plt.setp(plot2,linewidth=3)
plt.setp(plot3,linewidth=3)
ax.legend()
ax.set_ylabel(r'Moment')
ax.set_xlabel(r'Factor loading ($\lambda$): SSRS ($t=5$)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/lambda_t5.pdf', format='pdf')
plt.close()
