...
for i in range(size_grid): 
	param0.betaw[6,0] = p_list[i]
	emax_instance=output_ins.emax(param0,model)
	choices=output_ins.samples(param0,emax_instance,model)
	dic_betas=output_ins.aux_model(choices)
	target_moment[i] = np.mean(dic_betas['beta_wagep'][6,:],axis=0)
...
ax.legend()
ax.set_ylabel(r'$\rho$ observed wage regression',fontsize=font_size)
ax.set_xlabel(r'$\rho$ of wage offer process',fontsize=font_size)
...
ax.legend(loc=0)
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/rho_wage.pdf', format='pdf')
plt.close()
