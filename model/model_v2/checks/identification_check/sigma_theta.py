...
for i in range(size_grid): 
	param0.sigma2theta = p_list[i].copy()
	emax_instance=output_ins.emax(param0,model)
	choices=output_ins.samples(param0,emax_instance,model)
	dic_betas=output_ins.aux_model(choices)
	target_moment[i] = np.mean(dic_betas['beta_inputs'][4,:],axis=0)
...
ax.legend()
ax.set_ylabel(r'$Var(PCA)$')
ax.set_xlabel(r'Variance of production function process')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
ax.yaxis.set_ticks_position('left')
ax.xaxis.set_ticks_position('bottom')
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/checks/sigma_theta.pdf', format='pdf')
plt.close()
