...
for i in range(size_grid): 
	param0.gamma1 = p_list[i]
	emax_instance=output_ins.emax(param0,model)
	choices=output_ins.samples(param0,emax_instance,model)
	dic_betas=output_ins.aux_model(choices)
	target_moment_original[i] = np.mean(dic_betas['beta_inputs'][0,:],axis=0)
...
for i in range(size_grid): 
	...
	dic_betas=output_ins.aux_model(choices)
	target_moment_noeitc[i] = np.mean(dic_betas['beta_inputs'][0,:],axis=0)
...
ax.legend()
ax.set_ylabel(r'$Corr(SSRS_2, SSRS_5)$',fontsize=font_size)
ax.set_xlabel(r'Self-productivity ($\gamma_2$)',fontsize=font_size)
...
ax.legend(loc=0)
plt.show()
fig.savefig('/mnt/Research/nealresearch/new-hope-secure/newhopemount/results/Model/experiments/EITC/gamma1_check_eitc.pdf', format='pdf')
plt.close()