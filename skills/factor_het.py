...
class Factor:
	...
	def ll(self,param1):
		...
		lambda_m[1:(self.n_m),0]=self.lambda_m.copy() #factor loadings
		kappa_m=self.kappa_m.copy() #4 cutoffs x # measures
		...
		ind = ind+1
		mu_aux=param1[ind]
		mu=[np.array( [mu_aux, -p_aux*mu_aux/(1-p_aux)])]#mean of theta normalized to 0
	...
	def optimizer(self):
		...
		beta0 = np.concatenate((np.log(self.prob_0/(1-self.prob_0)),
			self.mu_0,np.log(self.vars_theta_0),
			np.log(self.prob_1/(1-self.prob_1)),self.mu_1,np.log(self.vars_theta_1)),axis=0)
