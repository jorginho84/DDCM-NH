...
It keeps constant cutoffs and estimates only mus and sigmas.
...
def gen_est(j):
	...
	#list of estimates: by year
	ests = []

	for k in range(3):

		output_ins = factor.Factor(n[k],n_m[k],n_r,M[k][list_sample[j][k],:],
			d_ra[k][list_sample[j][k],:],lambda_opt[k],
			kappa_opt[k],p_0[k],mu_0[k],sig_theta_0[k],
			p_1[k],mu_1[k],sig_theta_1[k])
		output=output_ins.optimizer()

		ests.append(output.x)
	

	return ests

#Estimation (4 samples in parallel)
print ''
print 'Start optimizer'
start=datetime.now()
pool = ProcessPool(nodes=4)
betas = pool.map(gen_est,range(4))
