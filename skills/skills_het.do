...
forvalues x=0/1{/*old,young*/
	...
	foreach variable of varlist `Y2_B1' `Y5_B2' `Y8_B2'{
		...
		gen dpr = pr_1 - pr_0
		qui: sum dpr
		local beta_model`x'_`variable' = string(round(r(mean),0.001),"%9.3f")
...
foreach variable of varlist `Y2_B1' `Y5_B2' `Y8_B2'{
	gen `variable'_aux= 1 if `variable'>=4
	replace `variable'_aux = 0 if `variable'<4
	drop `variable'
	rename `variable'_aux `variable'
}
...
xi: reg yhat2 i.p_assign if d_sample==0
xi: reg yhat5 i.p_assign if d_sample==1  
xi: reg yhat8 i.p_assign if d_sample==1
