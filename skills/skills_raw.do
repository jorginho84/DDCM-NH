...
foreach variable of varlist `Y2_B1' `Y2_B2'{
	...
	*Baseline
	...
	forvalues s = 0/1{/*the sample loop*/
		...
		*regressions
		qui: oprobit `variable' i.d_RA if d_young==`s'
		qui: predict xbetas, xb
		gen xbetas_tilde = xbetas - _b[1.d_RA]*d_RA
		gen pr_1 = 1-normal(_b[/cut3]- (xbetas_tilde + _b[1.d_RA]))
		gen pr_0 = 1-normal(_b[/cut3]- (xbetas_tilde))
		...
		qui: oprobit `variable' i.d_RA age_ra agechild i.marital i.ethnic d_HS higrade i.pastern2 if d_young==`s'
		qui: predict xbetas, xb
		gen xbetas_tilde = xbetas - _b[1.d_RA]*d_RA
		gen pr_1 = 1-normal(_b[/cut3]- (xbetas_tilde + _b[1.d_RA]))
		gen pr_0 = 1-normal(_b[/cut3]- (xbetas_tilde))
...
forvalues s = 0/1{
	...
	*write here the loop:
	foreach variable in `Y2_B1' {
		local lab: variable label `variable'
		file write Y2 "`lab' (\$n=`n_`variable'_`s''\$) &       & `baseline_`variable'' & "
